...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: admin.id }, process.env.JWT_SECRET, {
      expiresIn: process.env.NEXT_PUBLIC_TOKEN_COOKIE_MAX_AGE_IN_DAYS,
    });
    res.json({ token });
  } catch (err) {
...
app.post("/locations", async (req, res) => {
  const { location, history, description, map, tips, image, districts } =
    req.body;
  const query =
    "INSERT INTO location (location, history, description, map, tips, image, districts ) VALUES (?, ?, ?, ?, ?, ?, ?)";
...
app.put("/locationsEdit/:id", async (req, res) => {
  const { id } = req.params;
  const { location, history, description, map, tips, image, districts } =
    req.body;
  const query = `
    UPDATE location
    SET location = ?, history = ?, description = ?, map = ?, tips = ?, image =?, districts = ? 
    WHERE id = ?
  `;
